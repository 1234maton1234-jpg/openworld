...
For a building, obtain the current owned plots first. If none exists, ask the player to claim a plot in the game; never guess a boundary. When the server later permits multiple plots, select the requested entry by its `key` and use the matching `localPolygons` entry instead of silently choosing the first. Read [references/model-rules.md](references/model-rules.md) before modeling or export. Prefer live server numeric limits; the reference supplies format and placement constraints that the rules response does not describe.
