...
export async function run(args=process.argv.slice(2)){
  const options={};const words=[];for(let i=0;i<args.length;i++){if(args[i].startsWith('--')){const key=args[i].slice(2);if(!['server','title','config','help','token','no-browser'].includes(key))throw new Error('未知参数 --'+key);options[key]=['help','token','no-browser'].includes(key)?true:args[++i];if(options[key]===undefined)throw new Error('参数缺少值');}else words.push(args[i]);}
  if(!words.length||options.help){console.log('openworld login [--server URL]\nopenworld whoami\nopenworld logout\nopenworld avatar set FILE.glb\nopenworld vehicle set FILE.glb\nopenworld plot upload FILE.glb --title NAME\nopenworld plot submit DRAFT_ID\nlogin 默认打开浏览器授权；--no-browser 仅显示链接；--token 使用环境变量或标准输入 Token。');return;}
  ...
  const url=new URL(options.server||(words[0]==='login'?'https://mirako-openworld.zeabur.app':saved?.server)||'https://mirako-openworld.zeabur.app');if(url.username||url.password||url.pathname!=='/'||url.search||url.hash||!(url.protocol==='https:'||url.protocol==='http:'&&['127.0.0.1','localhost','[::1]'].includes(url.hostname)))throw new Error('服务器须为 HTTPS 源地址；仅本机允许 HTTP');const server=url.origin;
  ...
  const avatar=words[0]==='avatar'&&words[1]==='set',vehicle=words[0]==='vehicle'&&words[1]==='set',plot=words[0]==='plot'&&words[1]==='upload';if(!avatar&&!vehicle&&!plot)throw new Error('未知命令，使用 --help');if(!words[2])throw new Error('请指定 GLB 文件');if(plot&&!options.title)throw new Error('请指定 --title');const file=await stat(words[2]);if(!file.isFile()||file.size>12*1024*1024)throw new Error('模型须为不超过 12 MB 的文件');const body=await readFile(words[2]);const path=avatar?'/api/avatar':vehicle?'/api/vehicle':'/api/cli/plots/upload?title='+encodeURIComponent(options.title);console.log(JSON.stringify((await request(path,{method:avatar||vehicle?'PUT':'POST',body})).result,null,2));
}
