...
try{
  ...
  const mine=await get('/api/mine'),plots=Array.isArray(mine.plots)?mine.plots:mine.plot?[mine.plot]:[],plot=plots[0]||null;
  const localPolygons=plots.map(plot=>{const polygon=plot.polygon||[[plot.cx-plot.width/2,plot.cz-plot.depth/2],[plot.cx+plot.width/2,plot.cz-plot.depth/2],[plot.cx+plot.width/2,plot.cz+plot.depth/2],[plot.cx-plot.width/2,plot.cz+plot.depth/2]];return {key:plot.key||plot.x+','+plot.z,polygon:polygon.map(([x,z])=>[x-plot.cx,z-plot.cz])};});
  console.log(JSON.stringify({server:url.origin,user:session.user,plots,plot,plotLimit:mine.plotLimit??1,localPolygons,localPolygon:localPolygons[0]?.polygon||null,rules:authoring.rules,authoring,submissions:mine.submissions.map(({id,status,title,note})=>({id,status,title,note}))},null,2));
