...
async function session(){state.session=await api('/api/session');const user=state.session.user;$('#account-name').textContent=user?'@'+user.login:'访客';$('#login').hidden=!!user;$('#logout').hidden=!user;$('#mine').hidden=!user;$('#admin').hidden=!user?.admin;if(user){state.mine=await api('/api/mine');}else state.mine=null;renderProfile();renderSelected();}
