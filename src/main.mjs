...
const vehicleUpload=text('button','上传载具模型'),vehicleFile=document.createElement('input');vehicleFile.type='file';vehicleFile.accept='.glb,model/gltf-binary';vehicleFile.hidden=true;vehicleUpload.hidden=true;vehicleUpload.title='小汽车 GLB · 最大 12 MB · 宽 1.9 / 高 2 / 长 4.1 米';accountBar.append(vehicleUpload,vehicleFile);vehicleUpload.onclick=()=>vehicleFile.click();
vehicleFile.onchange=async()=>{const file=vehicleFile.files[0];if(!file)return;vehicleUpload.disabled=true;try{if(file.size>12*1024*1024)throw new Error('模型不能超过 12 MB');await api('/api/vehicle',{method:'PUT',headers:{'Content-Type':'model/gltf-binary'},body:file});world.recallVehicle();toast('载具模型已保存 · 按 P 召唤个人小汽车');}catch(error){toast(error.message);}finally{vehicleUpload.disabled=false;vehicleFile.value='';}};
async function session(){state.session=await api('/api/session');world.setPlayerIdentity(state.session.user?.id||null);const user=state.session.user;vehicleUpload.hidden=!user;$('#account-name').textContent=user?'@'+user.login:'访客';$('#login').hidden=!!user;$('#logout').hidden=!user;$('#mine').hidden=!user;$('#admin').hidden=!user?.admin;if(user){state.mine=await api('/api/mine');}else state.mine=null;renderProfile();renderSelected();}
...
function renderSelected(){
  if(landDraft){$('#land-login').hidden=!!state.session?.user;$('#plot-title').textContent='圈出你的领地';$('#plot-description').textContent='逐点画出 3～8 边形。绿色可领取，红色提示原因；地皮边界之间至少相隔 2 米。';$('#plot-owner').textContent='2 米网格 · 每条边至少 8 米 · 限高 600 米';$('#open-upload').hidden=true;$('#claim').disabled=!checkedLand||!!state.mine?.plot;$('#claim').textContent=state.mine?.plot?'你已拥有一块领地':checkedLand?'确认领取这片土地 ↗':'调整轮廓后领取';return;}
