...
import {minePlots,primaryPlot,remainingPlotClaims} from './owned-plots.mjs';
...
function renderProfile(){const user=state.session?.user,details=$('#profile-details'),plots=minePlots(state.mine);if(!details)return;document.querySelector('.profile-avatar').textContent=user?.login?.slice(0,1).toUpperCase()||'W';details.replaceChildren(...[['账号 ID',user?.id||'—'],['身份',user?.admin?'管理员':'小镇居民'],['领地',plots.length?(plots.length+' 块 · '+plots.reduce((sum,plot)=>sum+plot.area,0)+' m²'):'尚未领取']].map(([label,value])=>{const row=text('div','');row.append(text('span',label),text('strong',value));return row;}));}
...
function renderSelected(){
  ...
  $('#claim').disabled=!selected||!!row||remainingPlotClaims(state.mine)===0||!terrain?.buildable;
  $('#claim').textContent=!selected?'先选择一块地皮':own?'这是我的领地':row?'这块地已有主人':remainingPlotClaims(state.mine)===0?'当前已达到领地上限':state.session?.user?'领取这块地皮 ↗':'登录后领取地皮 ↗';
...
action('#blueprint',async()=>{const plot=primaryPlot(state.mine);if(!plot?.polygon){const a=document.createElement('a');a.href='/BUILDING_SPEC.md';a.download='openworldcraft-Codex建模规范.md';a.click();return;}
