...
const accountBar=document.querySelector('.account');accountBar.classList.add('account-hud');accountBar.setAttribute('aria-label','账号与领地');accountBar.append($('#mine'),$('#admin'));accountBar.hidden=true;document.body.append(accountBar);
...
async function session(){state.session=await api('/api/session');world.setPlayerIdentity(state.session.user?.id||null);const user=state.session.user;$('#account-name').textContent=user?'@'+user.login:'访客';$('#login').hidden=!!user;$('#logout').hidden=!user;$('#mine').hidden=!user;$('#admin').hidden=!user?.admin;if(user){state.mine=await api('/api/mine');}else state.mine=null;renderProfile();renderSelected();}
...
async function showAdmin(){location.href='/admin';}
