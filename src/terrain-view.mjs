...
export function createTerrain(scene,invalidate=()=>{}){
  ...
  return {rebuild,surface,ground:city.ground,lotInfo:city.lotInfo,lotAt:city.pick,registerTown,interact(raycaster){return townWaters.some(w=>w.interact(raycaster))||water.interact(raycaster);},pick(raycaster){return raycaster.intersectObjects([...group.children,...city.objects],false)[0]?.point;},update(dt,sun,paused){const start=performance.now();while(pending.length&&performance.now()-start<3)buildChunk(pending.shift());water.update(dt,sun,paused);townWaters.forEach(w=>w.update(dt,sun,paused));},get origin(){return origin;}};
}
