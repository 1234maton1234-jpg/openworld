...
export class CharacterShadow {
  ...
  constructor({
    ...
    scene,
    character,
    /** 太阳方向：从太阳指向场景（与线上 (-30,60,40) 的位置对应的是 (30,-60,-40)） */
     ...
     * 会被 far 平面切掉。
     *
     * 25 m 是按**线上真实的下限**定的：线上那盏灯的 y 被 `Math.max(5, y)`
     * 夹住（见 app.js 的天空更新），灯距 65 m ⇒ 最低高度角 atan(5/65)=4.4°，
     * 角色影子最长 1.7/tan(4.4°)=22.2 m。所以 25 是"现实里够用且只多不少"。
     */
    maxShadowLength = 25,
  }) {
   ...
   *
   * 这个窗口在测试台里几乎不存在（角色是同步建的，构造完立刻 attach），但**线上
   * 一定存在**：角色是 `/api/avatar/<name>.glb` 异步加载的。所以宁可多清这一趟。
   */
  _clearTarget() {
    const { renderer } = this;
  ...
  _updateCamera() {
    ...
    const clearance = Number.isFinite(this.groundHeight)
      ? Math.max(0, this._box.min.y - this.groundHeight) : 0;
    const tMax = d.y < -1e-4
      ? Math.min(span / -d.y, this.maxShadowLength) + clearance / -d.y
  ...
  /**
   * 换太阳方向。**线上太阳是动的**（天空系统每轮更新都改那盏灯的位置），
   * 不跟的话构造期克隆的那份就会过期 —— 影子朝着旧太阳、和场景光照脱开，
   * 而且 near/far 是按这个方向算的，一起错。接线上时每轮天空更新都要调。
   */
  setSunDirection(v) {
 ...
 *   · 叠在**基线**上（= 线上要落的形状，线上没有 CSM）→ 2048² 罩 300m
          ...
          `#include <lights_fragment_end>
{
  ...
  // 投的影子），但 directDiffuse 是**所有直射光之和** —— 夜里线上那 6 盏路灯
  // （PointLight，intensity = night×650，decay 2，且**不投影**）才是画面上唯一的
  // 强直射光，于是乘子把路灯的光也一起乘掉，在灯下的地面上挖出一块没有任何光源
  // 能解释的黑斑：场景阴影图里没有对应的影子（路灯不投影），只有这块地被压暗了。
  // 实测（?shadowdiff=1，21.5h/clear）：核心 138px 从 228 掉到 62，**只剩 27%**。
  //
