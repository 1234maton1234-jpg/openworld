...
export function createClimbing({probe,blocked,surface}){
  let attached=null,cooldown=0,approach=0;
  ...
  return {reset,get active(){return !!attached;},get yaw(){return attached?Math.atan2(attached.normal.x,attached.normal.z):null;},
    ...
    update(dt,{position,yaw,forward=0,right=0,enabled=true,paused=false,grounded=false}){
      ...
      const moving=position.distanceToSquared(before)>1e-8;return {handled:true,moving};
    }};
}
