...
export function createCockpit(){
  ...
  return {update(vehicle,enabled,keys,paused,dt,walking=true){
    ...
    summary.textContent=({car:'小汽车',bike:'单车',plane:'飞机',boat:'船'})[vehicle.type]+' · '+state.speed+' km/h';handbrakeHelp.hidden=!car&&vehicle.type!=='plane';handbrakeHelp.innerHTML=vehicle.type==='plane'?'<kbd>E / Q</kbd> 抬头 / 低头 · <kbd>A / D</kbd> 滚转':'<kbd>Shift</kbd> 手刹';if(vehicle.type==='plane')summary.textContent+=' · 高度 '+Math.round(vehicle.y)+' m';
