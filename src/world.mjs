...
export function createWorld({onSelect,onRegion,onError,onLandPoint,onLandMove}){
  ...
  function blocked(x,z,y){const h=floorAt(x,z);if(terrain.blocked(origin.x*70+x,origin.z*70+z,y-1.7)||h<.5||h>y-1.7+.8||vehicles.blocks(origin.x*70+x,origin.z*70+z,.35,y-1.7))return true;for(const {collision} of loaded.values())if(collision.blocked(x,z,y-1.7))return true;return false;}
