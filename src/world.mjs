...
export function createWorld({onSelect,onRegion,onError,onLandPoint,onLandMove}){
  ...
  function blocked(x,z,y){const h=floorAt(x,z);if(h<.5||h>y-1.7+.8||vehicles.blocks(origin.x*70+x,origin.z*70+z))return true;for(const {collision} of loaded.values())if(collision.blocked(x,z,y-1.7))return true;return false;}
  const vehicles=createVehicles(scene,{origin:()=>origin,surface:(x,z)=>{let h=terrain.surface(x,z);const limit=h+.45;for(const v of loaded.values())h=Math.max(h,v.collision.surface(x-origin.x*70,z-origin.z*70,limit));return h;},obstacle:(x,z,h,r)=>[...loaded.values()].some(v=>v.collision.blocked(x-origin.x*70,z-origin.z*70,h,r))});
  ...
  async function syncAvatar(){if(avatarLoading)return;avatarLoading=true;try{const response=await fetch('/api/avatar');if(!response.ok)return;const {url}=await response.json();if(url===avatarUrl)return;const model=url?(await loader.loadAsync(url)).scene:null;if(model)model.traverse(o=>{if(o.isMesh)o.castShadow=o.receiveShadow=true;});const old=avatar.setModel(model);if(old)dispose(old);avatarUrl=url;renderer.shadowMap.needsUpdate=true;}catch{}finally{avatarLoading=false;}}
  ...
  function renderView(dt){
    ...
    cockpit.update(vehicles.active,walking&&!thirdPerson,keys,paused,dt);
    ...
    avatar.update({position,yaw:v?v.heading:yaw,visible:walking&&thirdPerson,moving:walking&&!paused&&!seated&&!v&&['KeyW','KeyA','KeyS','KeyD'].some(k=>keys.has(k)),seated:!!seated||!!v,vehicleType:v?.type,crankPhase:v?.crankPhase||0,dt});
    if(!walking)return camera;
    viewCamera.copy(camera);viewCamera.aspect=camera.aspect;viewCamera.updateProjectionMatrix();
    if(v&&!thirdPerson){viewCamera.position.set(v.group.position.x,v.y+(v.type==='bike'?1.95:1.25),v.group.position.z);viewCamera.rotation.copy(camera.rotation);}
  ...
  function updateInteraction(now){
    if(vehicles.active){interactionHint.hidden=!walking||paused;interactionHint.textContent=`${VEHICLES[vehicles.active.type].name} · ${Math.round(Math.abs(vehicles.active.speed)*3.6)} km/h · W/S 前后 · A/D 转向 · 空格刹车${vehicles.active.type==='car'?' · Shift 手刹':''} · F 下车`;return;}
  ...
  function frame(now){requestAnimationFrame(frame);if(document.hidden||!ready){lastTime=now;return;}syncLandView();const dt=Math.min((now-lastTime)/1000||.016,.05);lastTime=now;
    ...
    updateInteraction(now);const renderCamera=renderView(dt);atmosphere.update(dt,renderCamera,walking&&paused);if(!walking){scene.fog=overviewFog;overviewFog.color.copy(scene.background);}terrain.update(dt,sun,reducedMotion||(walking&&paused));if(vehicles.active&&!thirdPerson)vehicles.active.group.visible=false;renderer.render(scene,renderCamera);if(vehicles.active)vehicles.active.group.visible=true;
  }
