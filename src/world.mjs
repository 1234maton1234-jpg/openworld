...
export function createWorld({onSelect,onRegion,onError,onLandPoint,onLandMove}){
  ...
  async function personalModel(){const response=await fetch('/api/vehicle?category='+vehicleType);if(!response.ok)throw new Error('无法读取载具模型');const row=await response.json();return {id:row.id,model:row.url?prepareCarModel((await loader.loadAsync(row.url)).scene,row.category):null};}
  ...
  function frame(now){requestAnimationFrame(frame);if(document.hidden||!ready||mapOpen){lastTime=now;return;}syncLandView();const dt=Math.min((now-lastTime)/1000||.016,.05);lastTime=now;
    ...
    walkMoved=climbState.moving;if(climbState.handled){velocity=0;if(climbing.yaw!==null)avatarYaw=climbing.yaw;}climbHud.hidden=!walking||paused||(!climbing.active&&climbing.stamina>=99.9);climbHud.querySelector('progress').value=climbing.stamina;climbHud.querySelector('#climb-help').textContent=climbing.active?'W/S 上下 · A/D 横移 · Space 蹬墙跳 · X 松手':'落地恢复体力';
    ...
    if(walking&&now-lastTelemetry>100){lastTelemetry=now;canvas.dataset.walkPosition=JSON.stringify({x:origin.x*PLOT.cell+camera.position.x,y:camera.position.y,z:origin.z*PLOT.cell+camera.position.z,ground:floorAt(camera.position.x,camera.position.z),climbing:climbing.active,stamina:climbing.stamina,paused});}
    ...
    updateInteraction(now);const renderCamera=renderView(dt);atmosphere.update(dt,renderCamera,walking&&paused);if(!walking){scene.fog=overviewFog;overviewFog.color.copy(scene.background);}terrain.update(dt,sun,reducedMotion||(walking&&paused),renderCamera);renderer.render(scene,renderCamera);
  }
