...
export function createAvatar(scene){
  ...
  function setModel(model,clips=[]){rig?.dispose();rig=null;const old=custom;if(old)root.remove(old);custom=model;if(model){const bounds=new T.Box3().setFromObject(model),size=bounds.getSize(new T.Vector3()),center=bounds.getCenter(new T.Vector3()),scale=1.8/size.y;const holder=new T.Group();holder.add(model);model.position.sub(new T.Vector3(center.x,bounds.min.y,center.z));holder.scale.setScalar(scale);custom=holder;root.add(holder);rig=createRiggedAvatar(model,clips);}return old;}
