 ...
 *
 * 为什么要单独一个入口：测试台（main.js）只覆盖了适配层**已经在用**的那部分 API ——
 * 每帧反推太阳、注入接收面、定位角色。而适配层里真正没被跑过的，是**面向宿主**的那几条：
 * 流式新材质补注入、换角色复用 RT、XR 守卫。这些恰恰是接线上最容易静默出错的地方
 * （错了不会有异常，只会"新开的那片地没有角色影子"）。
 *
 * 所以这里自己造一个最小宿主（地面 + 一个方盒子当角色 + 一盏平行光），
 * 逐条断言，把结果打成文本。无头跑法：
 *
 *   bash tools/headless.sh dom "http://localhost:PORT/adapter-check.html" /tmp/ac.html
 *   grep -oE "(PASS|FAIL) [^<]*" /tmp/ac.html
 *
...
/** 线上那盏灯的初值位置：(-30,60,40)，模长 78.1 ⇒ 高度角 50.19° */
...
// ── 1. 太阳方向反推（纯函数）────────────────────────────────
{
  ...
  check('反推 = -position 归一化（线上那盏灯的形状）', !!d && vecEq(d, want),
...
// ── 2. 没挂角色时不该画任何东西 ──────────────────────────────
// 线上角色是异步加载的，适配层从构造到 attachCharacter 之间会跑很多帧。
...

{
  ...
  // 整片地面被判成"全体在阴影里"直接黑掉。线上角色是异步加载的，这个窗口真实存在。
...
// ── 3. 太阳方向每帧反推（宿主不动任何一行适配层代码）──────────
{
  ...
  check('灯被挪走后下一帧就跟上（线上天空系统每轮都改灯位）',
...
adapter.attachScene(scene);
{
  ...
  // 模拟线上流式加载：跑着跑着一片新区块落地，带来**运行时新建的**材质。
  // 这不是假设 —— 线上地形/道路就是按区块现造 MeshStandardMaterial 的。
...
// ── 5. 跳过的材质要能被诊断出来 ──────────────────────────────
// 静默跳过是接线上最难查的故障：地上就是没影子，但没有任何报错。
{
...
// ── 7. 换角色：**必须复用同一个 RT** ────────────────────────
// 这是接线上换模型/换皮肤的那条路。已注入材质的 `charShadowMap` uniform 在编译期
...
// 像素级的结论在测试台（`?shadowdiff=1`，见 main.js 的 measureShadowDamage）：
// 白天必须 受影响>0（乘子还活着），夜里 21.5h 必须 受影响=0px（不再吃路灯）。
