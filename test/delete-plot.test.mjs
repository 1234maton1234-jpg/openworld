...
test('owner deletion releases land, removes versions and drafts, and preserves avatars and neighbors',async()=>{
  const dir=mkdtempSync(join(tmpdir(),'delete-plot-')),config={dataDir:dir,url:'http://127.0.0.1:8787',adminIds:[]}, {app,store}=createApp(config);
  const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));const base='http://127.0.0.1:'+server.address().port;
  try{
    const owner=testSession(store),other=testSession(store,'1002');store.claim('1001',-7,1);
    ...
    const publicPlot=store.world(-7,1,1).find(p=>p.owner==='1001');assert.equal(publicPlot.name,'My garden');assert.equal(publicPlot.description,'A quiet garden.');
    store.db.prepare('INSERT INTO plots(x,z,owner) VALUES (?,?,?)').run(900,900,'1002');
    const published=store.submit('1001','Published',{});store.review(published.id,'1001',true,'');const pending=store.submit('1001','Pending',{});
    store.db.prepare('INSERT INTO model_drafts VALUES (?,?,?,?,?)').run('draft','1001','Draft','{}',0);
    store.db.prepare('INSERT INTO avatars VALUES (?,?,?)').run('1001','avatar','{}');
    ...
    assert.equal(store.getPlot('1001'),undefined);assert.ok(store.getPlot('1002'));assert.equal(store.getSubmission(pending.id),undefined);
    for(const id of [published.id,pending.id,'draft'])assert.equal(existsSync(join(dir,'uploads',id+'.glb')),false);
    assert.equal(existsSync(join(dir,'uploads','avatar.glb')),true);assert.ok(store.db.prepare('SELECT 1 FROM avatars WHERE owner=?').get('1001'));
    assert.equal((await fetch(base+'/assets/'+published.id+'.glb')).status,404);
    assert.equal((await remove(headers)).status,404);assert.throws(()=>store.review(pending.id,'1001',true,''));
    assert.ok(store.claim('1001',-7,1));assert.equal(store.db.prepare('SELECT count(*) n FROM model_drafts').get().n,0);
