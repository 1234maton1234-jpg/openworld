...
test('GitHub token login uses verified GitHub identity and rejects invalid credentials',async()=>{
  const dir=await mkdtemp(join(tmpdir(),'openworld-auth-')),config={dataDir:dir,production:false,url:'http://127.0.0.1:8787',adminIds:[]},{app,store}=createApp(config),server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));config.url='http://127.0.0.1:'+server.address().port;
...
test('CLI session, avatar replacement and owned draft submission work end to end',async()=>{
  const dir=await mkdtemp(join(tmpdir(),'openworld-cli-')),config={dataDir:dir,production:false,url:'http://127.0.0.1:8787',adminIds:[]};const {app,store}=createApp(config),server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));config.url='http://127.0.0.1:'+server.address().port;
  ...
  try{
    ...
    let polygon;for(const lot of store.planner.around(0,0).lots){const x=Math.round(lot.cx/2)*2,z=Math.round(lot.cz/2)*2,p=[[x-16,z-16],[x+16,z-16],[x+16,z+16],[x-16,z+16]];try{store.checkLand(p);polygon=p;break;}catch{}}assert.ok(polygon);store.claimLand('1001',polygon);
    ...
    const visitor=testSession(store,'1002'),other={Origin:config.url,Cookie:visitor.cookie,'X-CSRF-Token':visitor.csrf};
    ...
    assert.equal(JSON.parse((await command('plot','submit',draft.id)).stdout).status,'pending');assert.equal(store.getPlot('1001').published,null);
    ...
    await command('login');store.db.prepare('UPDATE sessions SET expires=0').run();await assert.rejects(command('whoami'),/会话已过期/);
  }finally{globalThis.fetch=original;await new Promise(r=>server.close(r));store.close();await rm(dir,{recursive:true,force:true});}
});
