...
test('player layer controls both maps and restores the saved preference',t=>{
  const contexts=[],buttons=['plots','players'].map(key=>({dataset:{layer:key},setAttribute(name,value){this[name]=value;}}));
  ...
  globalThis.document={hidden:false,body:{append(){}},addEventListener(){},querySelector:()=>null,createElement(tag){
    ...
    if(tag==='dialog'){const c=canvas(),cache=new Map();e.querySelector=s=>s==='canvas'?c:s==='[data-layer="players"]'?buttons[1]:cache.get(s)||cache.set(s,element()).get(s);e.querySelectorAll=()=>buttons;e.showModal=()=>e.open=true;dialog=e;}return e;
  }};
  const world={mapPose:{x:700,z:-350,heading:0},mapPlayers:[{name:'Nearby',x:950,z:-350}],setMapOpen(){}};
  createWorldMap(world);assert.ok(!dialog.innerHTML.includes('data-layer="bridges"'));assert.ok(!dialog.innerHTML.includes('data-layer="rivers"'));mini.onclick();draw();
  ...
  createWorldMap(world);assert.equal(buttons[1]['aria-pressed'],'false');
  buttons[1].onclick();assert.equal(saved.get('openworld:map-players'),'true');
});
