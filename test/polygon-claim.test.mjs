...
test('land without road frontage is buildable with a nullable entrance',async ()=>{
  const store=(await createStore(':memory:'));
  try{
    const plan={...(await store.planner.around(0,0)),roads:[],legacy:[]};let polygon,land;
...
test('polygon claims persist exact geometry and reject competing owners and legacy overlap',async ()=>{
  const store=(await createStore(':memory:'));try{const plan=(await store.planner.around(0,0));let polygon;
    for(const lot of plan.lots){const x=Math.round(lot.cx/2)*2,z=Math.round(lot.cz/2)*2,p=[[x-32,z-24],[x-24,z-32],[x+24,z-32],[x+32,z-24],[x+32,z+24],[x+24,z+32],[x-24,z+32],[x-32,z+24]];try{(await store.checkLand(p));polygon=p;break;}catch{}}
    assert.ok(polygon,'a buildable roadside polygon must exist');(await store.upsertUser('a','a'));(await store.upsertUser('b','b'));const plot=(await store.claimLand('a',polygon));
    assert.deepEqual(plot.polygon,polygon);assert.equal(plot.polygon.length,8);assert.ok(plot.area<4096);assert.deepEqual((await store.getPlot('a')),plot);
    (await assert.rejects(async ()=>(await store.claimLand('b',polygon)),/公共通道/));(await assert.rejects(async ()=>(await store.claimLand('a',polygon)),/每个账号/));
    assert.deepEqual((await store.world(Math.round(plot.cx/70),Math.round(plot.cz/70),4))[0].polygon,polygon);
  }finally{(await store.close());}
});
...
test('authenticated HTTP polygon claim is revalidated and visible to visitors',async()=>{
  ...
  const config={dataDir:dir,production:false,url:'http://127.0.0.1:8787',adminIds:[]},{app,store}=(await createApp(config)),server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));const base=`http://127.0.0.1:${server.address().port}`;
  try{let polygon;for(const lot of (await store.planner.around(0,0)).lots){const x=Math.round(lot.cx/2)*2,z=Math.round(lot.cz/2)*2,p=[[x-32,z-32],[x+32,z-32],[x+32,z+32],[x-32,z+32]];try{(await store.checkLand(p));polygon=p;break;}catch{}}assert.ok(polygon);
    const auth=(await testSession(store)),cookie=auth.cookie,headers={Origin:config.url,Cookie:cookie,'X-CSRF-Token':auth.csrf,'Content-Type':'application/json'};
    ...
    (await store.db.exec('PRAGMA wal_checkpoint(FULL)'));const second=(await createStore(join(dir,'town.sqlite')));try{assert.deepEqual((await second.getPlot(plot.owner)).polygon,polygon);}finally{(await second.close());}
