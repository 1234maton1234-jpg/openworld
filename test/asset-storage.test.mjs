...
test('avatar and all vehicle uploads round-trip through R2-backed HTTP routes',async t=>{
  ...
  const submission=await fetch(config.url+'/api/submissions?title=R2-building',{method:'POST',headers,body:bytes});assert.equal(submission.status,201);const building=await submission.json();await ensureModelVariants(remoteStore.store,building.id);
