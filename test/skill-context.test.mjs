...
test('skill context returns owned geometry and rules without leaking session credentials',async()=>{
  const dir=mkdtempSync(join(tmpdir(),'skill-context-')),config={dataDir:dir,url:'http://127.0.0.1:8787',adminIds:[]},{app,store}=createApp(config),server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));
  try{const auth=testSession(store),url='http://127.0.0.1:'+server.address().port,path=join(dir,'credentials.json');store.claim('1001',-7,1);writeFileSync(path,JSON.stringify({server:url,...auth}));
