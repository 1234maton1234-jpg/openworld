...
test('layout migration preserves ownership and published buildings and runs once',()=>{
  const dir=mkdtempSync(join(tmpdir(),'town-layout-')),path=join(dir,'town.sqlite');let s;
  try{
    s=createStore(path);s.upsertUser('existing','owner');
    s.db.prepare('INSERT INTO plots(x,z,owner,published,elevation) VALUES (2,0,?,?,5.1)').run('existing','building-id');
    s.db.exec("DELETE FROM world_meta WHERE key='plot-layout'");s.close();
    s=createStore(path);const row=s.getPlot('existing');
    ...
    s.close();s=createStore(path);assert.deepEqual(s.getPlot('existing'),row);
