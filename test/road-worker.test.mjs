...
test('road worker coalesces movement and discards stale geometry',()=>{
  ...
  try{
    ...
    const first=worker.sent[0];worker.onmessage({data:{id:first.id,result:roadMarkings(first.roads)}});
    assert.equal(worker.sent.length,2);assert.equal(view.objects.length,0);
    const latest=worker.sent[1];worker.onmessage({data:{id:latest.id,result:roadMarkings(latest.roads)}});
