...
test('browser approval, denial, expiry, CSRF, one-use redemption and CLI persistence',async()=>{
 const dir=mkdtempSync(join(tmpdir(),'cli-oauth-')),config={dataDir:dir,url:'http://127.0.0.1:8787',adminIds:[],clientId:'test',clientSecret:'test'},{app,store}=createApp(config),server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));config.url='http://127.0.0.1:'+server.address().port;
 const auth=testSession(store),headers={Cookie:auth.cookie,'X-CSRF-Token':auth.csrf};
 ...
 const expired=await start();store.db.prepare('UPDATE cli_authorizations SET expires=0 WHERE code=?').run(expired.code);assert.equal((await post('/api/cli/authorize/poll',{secret:expired.secret})).status,410);
