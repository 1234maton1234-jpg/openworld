...
test('client exposes passenger targets and handles boarding, leaving and disconnect',t=>{
  ...
  receive({type:'welcome',userId:null});const pose={x:0,y:2,z:0,yaw:0,active:true};
  ...
  receive({type:'ride',ride:{owner:'owner',seat:1,pose}});ws.onclose({code:1006});assert.equal(client.ride,null);assert.equal(client.rideTargets().length,0);assert.equal(exits,2);
});
