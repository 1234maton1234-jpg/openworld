 ...
 * ── 它解决的宿主问题 ──────────────────────────────────────────────────
 *
 * 1. **太阳是动的**。宿主那盏带阴影盒的平行光由天空系统每轮改位置，
 *    夜里还会换成月亮方向，并且把最低高度角钳在一个很小的角度上。
 ...
 *    **下一帧补一次遍历**。不做成"每帧遍历整个场景" —— 宿主是流式开放世界，
 *    每帧 `scene.traverse()` 是 O(n)。
 *
 * 3. **宿主可能有不止一条渲染路径**（朴素 `renderer.render()`、多 pass 合成器）。
 *    两条路都汇进同一个 `render()` 方法 ⇒ `update()` 必须在**那次主渲染之前**调。
 *
 * 4. **WebXR**：宿主有 XR 管理器时，`xr.isPresenting` 下多跑一趟 `renderer.render()`
 *    会打乱 XR 的 framebuffer 绑定 ⇒ `update()` 里直接跳过。
 *
 * ── 接进宿主大概就是这样 ──────────────────────────────────────
 *
 *   const adapter = new CharacterShadowAdapter({ renderer, scene, sun: sunLight });
 *   adapter.attachScene(scene);                  // 接收面（地面等所有接影材质）
...
/** 参考档位「300m 阴影盒 / 2048²」的半影 = 14.6cm ⇒ 对齐它要 4.7（实测 14.69cm） */
...
export class CharacterShadowAdapter {
  ...
  constructor({
     ...
     * 占位投射体：**一个不在场景里的空组**。
     *
     * 为什么不直接拿宿主的角色：`attachCharacter()` 之前角色可能还没加载
     * （宿主角色通常是异步加载的），而 CharacterShadow 的构造器
     * 需要立刻有东西能量。给它一个空组，包围盒为空 ⇒ `update()` 自己早退，
     * 一次都不会画。
     *
     * **绝不能把宿主的角色 move 进一个包装组** —— 宿主角色挂在被浮动原点平移过的
   ...
   *
   * 挂到宿主已有的区块刷新钩子上：
   *   `createWorld(scene, () => { renderer.shadowMap.needsUpdate = true; adapter.markWorldDirty(); })`
   */
  markWorldDirty() {
    this._dirty = true;
    return this;
   ...
   * 默认 4.7 = 对齐参考档位（300m 阴影盒 / 2048² = 14.6cm，实测 14.69cm）。
   */
  setRadius(v) {
  ...
  /** 诊断。接进宿主后**把 `skipped` 打出来** —— 非 0 就说明有接收面收不到影子 */
  stats() {
    return {
 ...
 * 从一盏平行光的位置反推它照向哪。
 *
 * 那盏灯 `position` 是"离原点的太阳方位向量"，`target` 没有赋值 ⇒ 默认在
