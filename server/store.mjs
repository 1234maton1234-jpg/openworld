...
export async function createStore(path){
  ...
  const getPlot=async owner=>decode((await db.prepare('SELECT * FROM plots WHERE owner=?').get(owner)));
  ...
  async function claim(owner,x,z){coordinate(x);coordinate(z);return (await transaction(async ()=>{
    if((await getPlot(owner)))fail(409,'每个账号只能领取一块地皮');
  ...
  async function review(id,reviewer,approve,note=''){return (await transaction(async ()=>{
    ...
    if(approve){const old=(await getPlot(row.owner)).published;if(old)(await db.prepare("UPDATE submissions SET status='superseded' WHERE id=?").run(old));(await db.prepare('UPDATE plots SET published=? WHERE owner=?').run(id,row.owner));}
    (await db.prepare('UPDATE submissions SET status=?,reviewer=?,reviewed=?,note=? WHERE id=?').run(approve?'published':'rejected',reviewer,Date.now(),note,id));return (await getSubmission(id));
  }));}
  ...
  return {db,planner,transaction,getUser,getPlot,deletePlot,plotRevision,getSubmission,upsertUser,claim,claimLand,checkLand,canSubmit,submit,review,world,rate,close:async ()=>(await db.close())};
