...
export async function createPlanner(db){
  (await db.exec('CREATE TABLE IF NOT EXISTS city_regions(x INTEGER NOT NULL,z INTEGER NOT NULL,version INTEGER NOT NULL,plan TEXT NOT NULL,PRIMARY KEY(x,z)); CREATE TABLE IF NOT EXISTS city_legacy(x INTEGER NOT NULL,z INTEGER NOT NULL,cx REAL NOT NULL,cz REAL NOT NULL,elevation REAL NOT NULL,PRIMARY KEY(x,z))'));
  const saved=(await db.prepare("SELECT value FROM world_meta WHERE key='city-terrain'").get());
  ...
  if(!saved){await db.transaction(async()=>{(await db.exec('INSERT OR IGNORE INTO city_legacy SELECT x,z,x*70,z*70,elevation FROM plots'));(await db.prepare("INSERT INTO world_meta VALUES ('city-terrain',?)").run(String(CITY.terrainVersion)));});}
  const legacy=(await db.prepare('SELECT * FROM city_legacy').all()).map(p=>({...p,width:PLOT.width,depth:PLOT.depth,legacy:true}));
  let waterConfig=(await db.prepare("SELECT value FROM world_meta WHERE key='hydrology-v2'").get());
  if(!waterConfig){const frozen=(await db.prepare('SELECT plan FROM city_regions').all()).map(r=>JSON.parse(r.plan).bounds);(await db.prepare("INSERT INTO world_meta VALUES ('hydrology-v2',?)").run(JSON.stringify(frozen)));waterConfig={value:JSON.stringify(frozen)};}
  const frozen=JSON.parse(waterConfig.value);
  let coastalConfig=(await db.prepare("SELECT value FROM world_meta WHERE key='coastal-hydrology-v3'").get());
  if(!coastalConfig){const value=JSON.stringify({version:3,frozen:(await db.prepare('SELECT plan FROM city_regions').all()).map(r=>JSON.parse(r.plan).bounds),previous:{version:2,frozen}});(await db.prepare("INSERT INTO world_meta VALUES ('coastal-hydrology-v3',?)").run(value));coastalConfig={value};}
  const water=JSON.parse(coastalConfig.value),hydrology=createWorldHydrology(water);
  let terrainConfig=(await db.prepare("SELECT value FROM world_meta WHERE key='urban-terrain-v2'").get());
  if(!terrainConfig){const bounds=(await db.prepare('SELECT plan FROM city_regions').all()).map(r=>JSON.parse(r.plan).bounds);const value=JSON.stringify(bounds);(await db.prepare("INSERT INTO world_meta VALUES ('urban-terrain-v2',?)").run(value));terrainConfig={value};}
  let reliefConfig=(await db.prepare("SELECT value FROM world_meta WHERE key='regional-relief-v3'").get());
  if(!reliefConfig){const bounds=(await db.prepare('SELECT plan FROM city_regions').all()).map(r=>{const p=JSON.parse(r.plan),points=p.roads.flatMap(r=>r.points);return [Math.min(p.bounds[0],...points.map(v=>v[0])),Math.min(p.bounds[1],...points.map(v=>v[2])),Math.max(p.bounds[2],...points.map(v=>v[0])),Math.max(p.bounds[3],...points.map(v=>v[2]))];}),value=JSON.stringify(bounds);(await db.prepare("INSERT INTO world_meta VALUES ('regional-relief-v3',?)").run(value));reliefConfig={value};}
  ...
  let bridgeConfig=(await db.prepare("SELECT value FROM world_meta WHERE key='street-plan-v7'").get());
  if(!bridgeConfig){const old=(await db.prepare('SELECT plan FROM city_regions').all()).map(r=>JSON.parse(r.plan)),value=JSON.stringify({roads:[...new Map(old.flatMap(p=>p.roads).map(r=>[r.id,r])).values()],lots:old.flatMap(p=>p.lots)});(await db.prepare("INSERT INTO world_meta VALUES ('street-plan-v7',?)").run(value));bridgeConfig={value};}
  ...
  async function region(x,z){const saved=(await db.prepare('SELECT plan FROM city_regions WHERE x=? AND z=?').get(x,z));if(saved)return JSON.parse(saved.plan);
