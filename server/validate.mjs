...
export async function validateModel(bytes,plot=null){
  ...
  return new Promise((resolve,reject)=>{
    const worker=new Worker(new URL(import.meta.url),{workerData:{bytes,plot},resourceLimits:{maxOldGenerationSizeMb:128}});
...
if(!isMainThread){
  try{
    const {plot}=workerData,bytes=Buffer.from(workerData.bytes),length=bytes.readUInt32LE(12);
    ...
    if(json.animations?.length||json.skins?.length||(json.meshes||[]).some(m=>m.primitives.some(p=>p.targets?.length)))fail(400,'首版只接收静态建筑，不支持骨骼、动画或形变');
