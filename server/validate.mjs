...
if(!isMainThread){
  try{
    ...
    const resources=inspectResourceDeclarations(json);
    ...
    let triangles=0,primitives=0,vertices=0;const uniquePositions=new Set();
    scene.traverse(node=>{const mesh=node.getMesh();if(!mesh)return;for(const p of mesh.listPrimitives()){
      ...
      triangles+=(p.getIndices()?.getCount()??pos.getCount())/3;primitives++;vertices+=pos.getCount();uniquePositions.add(pos);
      if(triangles>RULES.maxTriangles||primitives>RULES.maxPrimitives||vertices>RULES.maxVertices)fail(400,'模型需在 10 万三角面、30 万顶点、200 个绘制单元以内');
    }});
    ...
    if(bounds.min.some((v,i)=>(i!==1||avatar||workerData.vehicle)&&Math.abs(v)>10000)||bounds.max.some((v,i)=>(i!==1||avatar||workerData.vehicle)&&Math.abs(v)>10000))fail(400,'请将建筑原点移到场景中心附近');
    ...
    for(const texture of root.listTextures()){
      ...
      pixels+=metadata.width*metadata.height;if(pixels>RULES.maxTexturePixels)fail(400,'全部贴图总像素需不超过 1600 万');
    ...
    const textureGpuBytesEstimate=Math.ceil(pixels*4*4/3);
    parentPort.postMessage({size,min:bounds.min,max:bounds.max,triangles,primitives,bytes:bytes.length,...(vehicleSeats?{vehicleSeats}:{}),resources:{version:1,category:avatar?'avatar':workerData.vehicle?(typeof workerData.vehicle==='string'?workerData.vehicle:'car'):'building',...resources,vertices,uniqueVertices:[...uniquePositions].reduce((n,p)=>n+p.getCount(),0),nodes:json.nodes?.length||0,skins:json.skins?.length||0,joints:(json.skins||[]).reduce((n,s)=>n+s.joints.length,0),textures:root.listTextures().length,texturePixels:pixels,textureGpuBytesEstimate}});
