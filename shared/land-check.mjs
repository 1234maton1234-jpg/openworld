...
export function createLandCheck(planning,owned=[]){
  ...
  return polygon=>{
    ...
    for(const p of [...owned,...(planning.legacy||[])])if(polygonDistance(polygon,plotPolygon(p))<LAND.gap)reject('与已领地皮之间须保留至少 8 米公共通道');
    ...
    let entrance,best=Infinity,frontage=0;
    for(const [a,b] of edges(polygon)){let run=0,previousNear=false;const length=Math.hypot(b[0]-a[0],b[1]-a[1]),count=Math.ceil(length/2);for(let i=0;i<=count;i++){
      const p=[a[0]+(b[0]-a[0])*i/count,a[1]+(b[1]-a[1])*i/count];let near=false;
      for(const s of paths){if(s.bridge)continue;const dx=s.b[0]-s.a[0],dz=s.b[1]-s.a[1],t=Math.max(0,Math.min(1,((p[0]-s.a[0])*dx+(p[1]-s.a[1])*dz)/(dx*dx+dz*dz||1))),q=[s.a[0]+dx*t,s.a[1]+dz*t],d=Math.hypot(q[0]-p[0],q[1]-p[1]);if(d-s.width/2>18||water.distance(...q)<48)continue;near=true;if(d<best){best=d;entrance={roadId:s.id,points:[[p[0],0,p[1]],[q[0],s.ay+(s.by-s.ay)*t,q[1]]]};}}
      run=near&&previousNear?run+length/count:0;previousNear=near;frontage=Math.max(frontage,run);
    }}
    if(!entrance||frontage<8)reject('至少 8 米边界须临公共道路，距路边不超过 18 米');
    const access=[entrance.points[0][0],entrance.points[0][2]],road=[entrance.points[1][0],entrance.points[1][2]];
    for(const p of owned)if(inside(plotPolygon(p),access)||edges(plotPolygon(p)).some(([a,b])=>segmentDistance(access,road,a,b)<4))reject('入口会穿过其他领地的公共间距，请更换临路侧');
    ...
    const roadY=entrance.points[1][1],elevation=Math.max(roadY-.03*best,Math.min(roadY+.03*best,(min+max)/2));
    if(Math.max(Math.abs(elevation-min),Math.abs(elevation-max))>.8)reject('地基与道路高差过大，请调整临路位置');
    entrance.points[0][1]=elevation+.04;
