import {SIDEWALK_WIDTH} from './street-style.mjs';
...
export function createUrbanTerrain(frozen=[],hydrology,config={version:2}){
  ...
  function height(x,z){const w=weight(x,z);if(!w)return cityHeight(x,z,hydrology);const d=hydrology?hydrology.distance(x,z):riverDistance(x,z),h=d<=14&&hydrology?.bedDepth?-hydrology.bedDepth(x,z):-1.5+(natural(x,z)+1.5)*smooth((d-14)/24);if(w===1)return h;const old=cityHeight(x,z,hydrology);return old+(h-old)*w;}
  function ground(x,z,lots=EMPTY,legacy=EMPTY,roads=EMPTY){
    ...
    if(changed){previousRoads=roads;const segments=roads.flatMap(r=>(r.sections||[r]).filter(s=>s.kind!=='crossing').flatMap(s=>s.points.slice(1).map((b,i)=>({a:s.points[i],b,width:s.width||r.width}))));for(const lot of lots)if(lot.version>=4&&lot.entrance){const [a,b]=lot.entrance.points;segments.push({a,b,width:6,access:true});}for(const s of segments)s.blend=s.access?18:Math.min(240,Math.max(24,(Math.max(s.a[1],s.b[1])+2)*3));nearbyRoads=index(segments,({a,b,width,blend})=>{const pad=width/2+SIDEWALK_WIDTH+blend;return [Math.min(a[0],b[0])-pad,Math.min(a[2],b[2])-pad,Math.max(a[0],b[0])+pad,Math.max(a[2],b[2])+pad];},128);}
    const bankDistance=hydrology?hydrology.distance(x,z):riverDistance(x,z);
    let h=bankDistance>14?riverbankHeight(bankDistance,base(x,z)):height(x,z),sum=0,total=0,strongest=0;
    ...
    for(const {a,b,width,access,blend} of nearbyRoads(x,z)){const dx=b[0]-a[0],dz=b[2]-a[2],length=dx*dx+dz*dz;if(length<1e-8)continue;const t=Math.max(0,Math.min(1,((x-a[0])*dx+(z-a[2])*dz)/length)),d=Math.hypot(x-a[0]-t*dx,z-a[2]-t*dz)-width/2-(access?2:SIDEWALK_WIDTH),y=a[1]+t*(b[1]-a[1])-.035,w=(1-smooth(d/blend))*(access?1:smooth((bankDistance-14)/28)),delta=(y-h)*w;raise=Math.max(raise,delta);cut=Math.min(cut,delta);if(d<=0&&w===1)ceiling=Math.min(ceiling,y);}
    h+=raise+cut;
