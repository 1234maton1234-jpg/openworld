...
import {directedRoute} from './directed-route.mjs';
...
export function createBridgePlanner(hydrology,field,{roads:fixed=[],lots:protectedLots=[]}={}){
  ...
  function junction(p){const id=key(p);if(nodes.has(id))return nodes.get(id);if(nodes.size>8192){nodes.clear();for(const [k,v] of fixedNodes)nodes.set(k,v);}let best=null,score=Infinity;
    const shared=coastal&&wet(p)<150,center=shared?[Math.round(p[0]/STEP)*STEP,0,Math.round(p[2]/STEP)*STEP]:p,step=shared?STEP:16;
    const radius=coastal?1536:384;for(let dx=-radius;dx<=radius;dx+=step)for(let dz=-radius;dz<=radius;dz+=step){const q=height([center[0]+dx,0,center[2]+dz]),d=(q[0]-p[0])**2+(q[2]-p[2])**2;if(d>=score)continue;if(allowed(q,HEAD)){best=q;score=d;}}
  ...
  function search(start,end,margin){
    ...
    const landward=new Map();
    for(const s of crossings){const a=s.id+':a',b=s.id+':b';points.set(a,s.points[0]);points.set(b,s.points.at(-1));for(const [id,head,inner] of [[a,s.points[0],s.points[1]],[b,s.points.at(-1),s.points.at(-2)]]){const length=distance(head,inner);landward.set(id,[(head[0]-inner[0])/length,(head[2]-inner[2])/length]);}connect(a,b,s);connect(b,a,{...s,points:s.points.toReversed(),deckStart:s.points.length-1-s.deckEnd,deckEnd:s.points.length-1-s.deckStart});}
    ...
    const special=[...points.keys()];for(const id of special){const p=points.get(id),gx=Math.round(p[0]/STEP),gz=Math.round(p[2]/STEP),candidates=[],outward=landward.get(id);for(let dx=-2;dx<=2;dx++)for(let dz=-2;dz<=2;dz++){const g=grid(gx+dx,gz+dz);if(!g)continue;const q=points.get(g);if(coastal&&outward&&((q[0]-p[0])*outward[0]+(q[2]-p[2])*outward[1]<Math.max(1,distance(p,q)*.5)))continue;if(clear(p,q))candidates.push(g);}candidates.sort((a,b)=>distance(p,points.get(a))-distance(p,points.get(b))||a.localeCompare(b));for(const g of coastal?candidates.slice(0,1):candidates){connect(id,g);connect(g,id);}}
    if(!coastal&&clear(start,end))connect('s','t');
    const anchor=[Math.round((start[0]+end[0])/512)*256,0,Math.round((start[2]+end[2])/512)*256];
    return directedRoute('s','t',id=>points.get(id),id=>{
      ...
      return edges.map(edge=>({...edge,penalty:edge.span?90+distance(edge.span.points[Math.floor(edge.span.points.length/2)],anchor):0}));
    });
  }
  function route(road){if(saved.has(road.id))return saved.get(road.id);if(cache.has(road.id))return cache.get(road.id);if(cache.size>2048)cache.clear();
    ...
    if(distance(start,end)<.01)return finish([{kind:'land',points:[start]}]);
    ...
    const shoreline=coastal&&shifted.some((p,i)=>wet(p)<150||i&&samples(shifted[i-1],p,32).some(q=>wet(q)<150));
    if(!shoreline&&shifted.every((p,i)=>!i||clear(shifted[i-1],p)))return finish([{kind:'land',points:shifted}]);
    ...
    function flush(){if(land.length<2)return;const simple=[land[0]];for(let i=0;i<land.length-1;){let j=coastal?i+1:land.length-1;while(j>i+1&&!clear(land[i],land[j]))j--;if(distance(simple.at(-1),land[j])>.01)simple.push(land[j]);i=j;}if(simple.length<2)return;const points=simple.slice(1).flatMap((p,i)=>samples(simple[i],p,8).slice(i?1:0).map(height));points[0]=land[0];points[points.length-1]=land.at(-1);sections.push({kind:'land',points});}
