...
export function createRoadNavigator(roads){
  ...
  return {route(start,target){
    ...
    if(from.edge===to.edge)return {status:'ok',points:[from.p,to.p],distance:distance(from.p,to.p),startGap:from.gap,endGap:to.gap};
    ...
    let visited=0;
    while(heap.items.length){const current=heap.pop(),{node,cost}=current;if(cost!==costs.get(node))continue;if(node===goal){const points=[to.p];let cursor=previous.get(goal);while(cursor){points.push(cursor.p);cursor=previous.get(cursor);}points.push(from.p);points.reverse();return {status:'ok',points,distance:cost,startGap:from.gap,endGap:to.gap,visited};}
